...
const isAnInappropiateMessage = message => {
	...
	if (
		messageToLowerCase.includes('feo') ||
		messageToLowerCase.includes('tonto')
	) {
...
const getInitials = (name, surname) => {
	const nameInitial = name.toUpperCase(name.substring(0));
	const surnameInitial = surname.toUpperCase(surname.charAt(0));
	console.log(`${nameInitial}.${surnameInitial}`);
};
...
const getRandomPassword = digits => {};
